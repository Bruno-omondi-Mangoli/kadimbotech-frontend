...
export default function AIChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "Hi! I'm Kadi, your Kadimbotech assistant 👋\n\nI can help you learn about our services, pricing, and how we can help your business. What would you like to know?",
    },
  ]);
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: data.reply || "Sorry, I could not process that. Please contact us directly." },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "Sorry, something went wrong. Please email us at kadimbotechsolutions@gmail.com." },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      >
        ...
        {/* Pulse ring */}
        {!open && (
          <span className="absolute w-14 h-14 rounded-full animate-ping opacity-30" style={{ background: "var(--color-teal)" }} />
        )}
      </button>
      ...
      {open && (
        <div
          ...
          style={{ height: "480px", background: "white", border: "1px solid var(--color-border-brand)" }}
        >
          {/* Header */}
          <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0" style={{ background: "var(--color-navy)" }}>
            <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0" style={{ background: "var(--color-teal)" }}>
              K
            </div>
            <div>
            ...
            <div className="px-4 py-2 flex flex-wrap gap-2 flex-shrink-0" style={{ borderTop: "1px solid var(--color-border-brand)", background: "white" }}>
              {suggestedQuestions.map((q) => (
                <button
                  ...
                  style={{ borderColor: "var(--color-teal)", color: "var(--color-teal)", background: "rgba(34,199,184,0.05)" }}
                >
                  {q}
                </button>
              ))}
